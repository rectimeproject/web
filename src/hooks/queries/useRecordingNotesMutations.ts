...
export const useCreateRecordingNoteMutation = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      return await recordingNotes.createRecordingNote(recordingId, durationOffset);
    },
    ...
    onError: (_err: unknown, variables: { recordingId: string; durationOffset: number }, context: { previous: IRecordingNote[] | undefined } | undefined) => {
      // Rollback on error
      if (context?.previous) {
        queryClient.setQueryData(
    ...
    onSettled: (_data: unknown, _error: unknown, variables: { recordingId: string; durationOffset: number }) => {
...
export const useDeleteRecordingNoteMutation = () => {
  ...
  return useMutation({
    ...
    onError: (_err: unknown, _variables: { noteId: string }, context: { previous: IRecordingNote[] | null; recordingId: string | null } | undefined) => {
      // Rollback on error
    ...
    onSettled: (_data: unknown, _error: unknown, _variables: { noteId: string }, context: { previous: IRecordingNote[] | null; recordingId: string | null } | undefined) => {
      // Refetch to ensure consistency
